...
🤗 Accelerate의 AWS SageMaker 통합을 보여주는 예제입니다. `accelerate config`를 지정하고 `accelerate launch` 🚀를 실행하기만 하면 됩니다. 정말 간단합니다!

1. `accelerate config --config_file accelerate_config.yaml`을 실행하여 accelerate 구성을 설정하고 SageMaker 질문에 답변합니다.

2. 다음은 🤗 SageMaker 추정기를 사용하여 학습 작업을 시작하기 위해 aws `profile`을 사용하는 샘플 구성입니다. 또한 필요한 SageMaker 권한이 지정된 `iam_role_name`도 있습니다. 이 구성에서는 사용자가 해당 AWS 설정에 따라 지정해야 하므로 `xxxxx`로 대체됩니다.
...
use_cpu: false
```
3. accelerate 구성 설문지를 통해 공식 🤗 DLC 대신 사용자 지정 도커 이미지를 지정할 수 있습니다. 이 항목이 제공되지 않으면 최신 공식 🤗 DLC가 사용됩니다.

4. TSV 파일을 통해 S3 데이터 위치를 가리키는 입력 채널 지원, 예를 들어 다음은 accelerate 구성 설정의 일부로 위치가 지정된 sagemaker_inputs.tsv의 내용입니다.
```tsv
...
5. TSV 파일을 통한 SageMaker 메트릭 로깅 지원, 예를 들어 다음은 accelerate 구성 설정의 일부로 위치가 지정된 sagemaker_metrics_definition.tsv의 내용입니다.
```tsv
...
6. 위의 기능이 설정된 accelerate 구성 예제 [XXXXX 값은 AWS 계정별 값임]:
```yaml
base_job_name: accelerate-sagemaker-1
compute_environment: AMAZON_SAGEMAKER
distributed_type: DATA_PARALLEL
...
use_cpu: false
```
7. 학습 스크립트를 실행하는 데 필요한 모든 라이브러리가 포함된 `requirements.txt`를 넣습니다.

8. s3 데이터세트를 사용하여 `text-classification` 예제 실행 (루트 디렉터리에서):
```bash
